 ...
 * Tested 31/08/21. Needs to be compiled in MATLAB as a MEX file
 * For example using the commands:
 * "mex -setup C; mex modelSolveAgeUnionsFrequency.c -R2018a"
 ****************************************************************************************************/
...
//     Define parameters
    double MJ, FJ, MA, FA, U, births, NJ, Nu, NA, N, xi;
...
//     Note: if you have a birth term, ensure it is greater than zero
    births = FMAX(p->b*U,TINY);
    
    xi = p->b*U-(p->uMJ*MJ + p->uFJ*FJ + p->uMA*MA + p->uFA*FA + (p->uMA + p->uFA)*U); 
    
//     Calculate derivatives
    dxdt[0] = births*p->s - (p->uMJ + p->tau)*MJ - MJ*xi;
    dxdt[1] = births*(1-p->s) - (p->uFJ + p->tau)*FJ - FJ*xi;
    dxdt[2] = p->tau*MJ + (p->uFA + p->d)*U - p->uMA*MA - (p->delta*MA*FA)/FMAX(Nu,TINY) - MA*xi;
    dxdt[3] = p->tau*FJ + (p->uMA + p->d)*U - p->uFA*FA - (p->delta*MA*FA)/FMAX(Nu,TINY) - FA*xi;
    dxdt[4] = (p->delta*MA*FA)/FMAX(Nu,TINY) - (p->uMA + p->uFA + p->d)*U - U*xi;
}
